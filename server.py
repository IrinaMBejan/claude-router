...
def get_pricing_per_request() -> Decimal:
    """Get the pricing per request from pyproject.toml."""
    try:
        with open("pyproject.toml", "rb") as f:
            config = tomllib.load(f)

        pricing = config.get("tool", {}).get("claude", {}).get("pricing_per_request", 0.1)
        return Decimal(str(pricing))
...
def handle_completion_request(
    ...
    logger.info(f"Processing completion request: <{ctx.id}>from <{request.sender}>")
...
def handle_chat_completion_request(
    ...
    logger.info(f"Processing chat request: <{ctx.id}>from <{request.sender}>")
    provider = load_router()
    accounting_client = accounting.get_or_init_user_client()
    pricing_per_request = get_pricing_per_request()
    logger.info(f"Pricing per request: {pricing_per_request}")
    try:
        transaction = accounting_client.create_delegated_transaction(
            amount=float(pricing_per_request), senderEmail=request.sender, token=request.accounting_token
        )
    ...
    try:
        ...
        # Only confirm transaction if generation was successful
